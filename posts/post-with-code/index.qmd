...
![](diseases_surveillance_system.png)
...
This is where I will further describe the completeness.

Try linking the the figures intext with figure number.
...
# Create Week and Count, ie, health facility reports recieved per week
weekly_count1 <- phess_clean %>% 
  mutate(epi_week = isoweek(end_date)) %>% 
  count(epi_week,health_facility, name = "reports_recieved")

# Create week and count, inlcuding facility not reporting during the epi week 
# as Na
...
# Creat Heat plot for reports recieved per Health Facilities per Epi Week
...
## SYNDROMES *(Still a work in progress)*
...
all_syndromes <- phess_clean %>%
  select(district,reporting_date, suspect_cholera:leprosy_cases) %>% 
...
```{r fig.height= 15}
# Join all_synd_pivot_long_format with facility count
combined <- 
  facility_count %>% 
  select(-simple_cough) %>% 
left_join(all_synd_pivot_long_format, by = "district")

# syndromesbyHF <-
  combined %>%
  group_by(district, health_facility,reporting_date, variables, syndromes,values) %>%
  summarise(total = sum(values, na.rm = TRUE)) %>%
  ggplot(aes(x = fct_reorder(health_facility, total), y = total, fill = variables)) +
  # geom_col()+
  geom_col(position = position_dodge(width = 0.9), width = 1) +
  # facet_wrap(~district, scales = "free",
  #            ncol = 1) +
    facet_grid(syndromes ~ district, scales = "free") +
  theme_bw() +
  labs(
    title = "Syndromes Categories",
    subtitle =str_glue("Reports recieved as of epi week {epi_week}",
